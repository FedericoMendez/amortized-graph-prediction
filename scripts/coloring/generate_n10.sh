#!/bin/bash
#SBATCH --job-name=coloring-n10-generate
#SBATCH --nodes=1
...
' "${OUTPUT_DIR}/manifest.json" "${COLORING_SEED}" "${CAP}" "${IMAGE_RESOLUTION}"; then
    echo "[coloring] reusing compatible N10 dataset at ${OUTPUT_DIR}"
    exit 0
  fi
  echo "Existing N10 dataset is incompatible with this experiment: ${OUTPUT_DIR}" >&2
